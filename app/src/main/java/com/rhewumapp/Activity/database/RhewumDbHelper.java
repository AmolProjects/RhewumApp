...
public class RhewumDbHelper extends OrmLiteSqliteOpenHelper {
    ...
    //added new
    public Dao<VibCheckerSummaryDao, Integer> getDominantFrequencyDao() throws SQLException {
        return getAccelerometerDao(); // Same table, but method provides semantic clarity.
    }
    ...
    //added for vibchecker
    public void deleteVibcheckerList(ArrayList<VibCheckerSummaryDao> arrayList) {
    ...
    public void maxAccelerometerData(float maxAccX,float maxAccY,float maxAccZ,String measurementTotalTime,String measurement_date,String image){
        try {
...
//            accelerometer.measurementTotalTime=measurementTotalTime;
            accelerometer.measurementDate = new Date();
    ...
    public void maxDominantFrequencyData(float maxdominantFrX,float maxdominantFrY,float maxdominantFrZ){
        try {
            VibCheckerSummaryDao accelerometerdata = new VibCheckerSummaryDao();
            accelerometerdata.dominantFrequencyX = maxdominantFrX;
            accelerometerdata.dominantFrequencyY = maxdominantFrY;
            accelerometerdata.dominantFrequencyZ = maxdominantFrZ;
            getAccelerometerDao().create(accelerometerdata);
    ...
    //added new method
    public List<VibCheckerSummaryDao> maxDominantFrequencyData() {
        List<VibCheckerSummaryDao> maxAccelerationData = new ArrayList<>();
        try {
            QueryBuilder<VibCheckerSummaryDao, Integer> queryBuilder = getAccelerometerDao().queryBuilder();
            queryBuilder.orderBy("xAxis", false); // Order descending for max value
            queryBuilder.orderBy("yAxis", false);
            queryBuilder.orderBy("zAxis", false);
            queryBuilder.limit(1L); // Get only the top entry
            maxAccelerationData = getAccelerometerDao().query(queryBuilder.prepare());
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return maxAccelerationData;
    }

//    public void insertVibCheckerData(float accX, float accY, float accZ, float dominantFrX, float dominantFrY, float dominantFrZ, float ampX,float ampY,float ampZ,int measurementTotalTime, int delay ,String measurementDate) {
//        try {
...
//            Utils.showLog("Data inserted: Acc: (" + accX + ", " + accY + ", " + accZ + ") | Dominant Fr: (" + dominantFrX + ", " + dominantFrY + ", " + dominantFrZ + ")");
//        } catch (SQLException e) {
//            Utils.showLog("Error inserting data: " + e.getMessage());
//        }
//    }
    ...
    public void insertVibCheckerData(float accX, float accY, float accZ, float dominantFrX,
                                     float dominantFrY, float dominantFrZ,
                                     float ampX, float ampY, float ampZ,
                                     int measurementTotalTime, int delay,
                                     String measurementDate, byte[] sensorData) {
        try {
            VibCheckerSummaryDao data = new VibCheckerSummaryDao();
            // Insert accelerometer data
            data.xAxis = accX;
    ...
    //added for xRaw Values
//    public List<VibCheckerSummaryDao> getLatestRawValues(int limit) {
//        List<VibCheckerSummaryDao> rawValues = new ArrayList<>();
//        try {
//            // Ensure the limit is greater than 0
//            if (limit <= 0) {
//                return rawValues;
...
//            QueryBuilder<VibCheckerSummaryDao, Integer> queryBuilder = getVibCheckerDao().queryBuilder();
//
//            // Order by ID in descending order to fetch the latest entries
//            queryBuilder.orderBy("id", false); // 'false' for descending order
//
//            List<VibCheckerSummaryDao> vibCheckerList = queryBuilder.query();
//
//            for (VibCheckerSummaryDao dao : vibCheckerList) {
//                Log.d("DB_INSERT", "RD:Fetched Data: xRawValue=" + dao.xRawValue +
//                        ", yRawValue=" + dao.yRawValue +
//                        ", zRawValue=" + dao.zRawValue);
//            }
//
//            // Limit the number of results
//            queryBuilder.limit((long) limit);
...
//
//            // Execute the query and retrieve the results
//            rawValues = getVibCheckerDao().query(queryBuilder.prepare());
//
//        } catch (SQLException e) {
//            e.printStackTrace();
//        }
...
//    }
    ...
    //added
//    public Dao<VibCheckerSummaryDao, Integer> getVibCheckerDao() {
//        try {
//            // Retrieve the DAO for VibCheckerSummaryDao
//            return getDao(VibCheckerSummaryDao.class);
//        } catch (SQLException e) {
//            e.printStackTrace();
//            return null;
//        }
